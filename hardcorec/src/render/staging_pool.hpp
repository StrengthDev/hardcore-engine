...
namespace ENGINE_NAMESPACE {
    ...
    public:
        ...
        upload_pool(VkDevice device, device_heap_manager &heap_manager, VkDeviceSize size) :
                staging_pool(device, heap_manager, size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
    ...
    private:
        enum conversion_type : u8 {
            ...
            // Simple buffer to image conversion
            BUFFER_TO_IMAGE = 0,
            ...
            // Buffer to depth stencil conversion
            BUFFER_TO_DEPTH_OR_STENCIL,

            // Image is converted to a completely different image
            FULL_CONVERSION,

            // Multisampled to so single sample (regular) image
            MULTISAMPLED_TO_SIMPLE,

            // Multisamped to multisampled conversion
