...
namespace ENGINE_NAMESPACE {
    ...
    inline VkFormat to_vk_format(const data_layout::value &type) {
        ...
        switch (type.t) {
            case data_layout::type::SCALAR:
                switch (type.ct) {
                    case component_t::FLOAT16:
                        format = VK_FORMAT_R16_SFLOAT;
                        break;
                    case component_t::FLOAT32:
                        format = VK_FORMAT_R32_SFLOAT;
                        break;
                    case component_t::FLOAT64:
                        format = VK_FORMAT_R64_SFLOAT;
                        break;
                    case component_t::INT8:
                        format = VK_FORMAT_R8_SINT;
                        break;
                    case component_t::INT16:
                        format = VK_FORMAT_R16_SINT;
                        break;
                    case component_t::INT32:
                        format = VK_FORMAT_R32_SINT;
                        break;
                    case component_t::INT64:
                        format = VK_FORMAT_R64_SINT;
                        break;
                    case component_t::UINT8:
                        format = VK_FORMAT_R8_UINT;
                        break;
                    case component_t::UINT16:
                        format = VK_FORMAT_R16_UINT;
                        break;
                    case component_t::UINT32:
                        format = VK_FORMAT_R32_UINT;
                        break;
                    case component_t::UINT64:
    ...
    inline VkPipelineVertexInputStateCreateInfo to_pipeline_inputs(const std::vector<data_layout> &layouts) {
        ...
        VkVertexInputBindingDescription *binding_descriptions = t_calloc<VkVertexInputBindingDescription>(
                layouts.size());
    ...
    inline VkDescriptorType to_vk_descriptor_type(shader::descriptor_t type) {
        switch (type) {
            case ENGINE_NAMESPACE::shader::UNIFORM:
                return VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            case ENGINE_NAMESPACE::shader::STORAGE:
                return VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
            case ENGINE_NAMESPACE::shader::SAMPLER:
                return VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
            case ENGINE_NAMESPACE::shader::TEXTURE:
                return VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
            case ENGINE_NAMESPACE::shader::IMAGE:
                return VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
            case ENGINE_NAMESPACE::shader::SAMPLER_BUFFER:
                return VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER;
            case ENGINE_NAMESPACE::shader::IMAGE_BUFFER:
                return VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER;
            case ENGINE_NAMESPACE::shader::SAMPLER_SHADOW:
    ...
    inline void alloc_descriptor_sets(VkDevice &handle, u32 object_descriptor_capacity,
                                      const u32 *n_pool_sizes,
                                      VkDescriptorPoolSize *pool_sizes[pipeline_descriptor_sets],
        ...
        for (u32 i = 0; i < n_pool_sizes[0]; i++)
            pool_sizes_map[pool_sizes[0][i].type].descriptorCount +=
                    pool_sizes[0][i].descriptorCount * object_descriptor_capacity;
        ...
        VK_CRASH_CHECK(vkCreateDescriptorPool(handle, &pool_info, nullptr, out_descriptor_pool),
        ...
        if (n_pool_sizes[0])
            for (u32 f = 0; f < max_frames_in_flight; f++)
                for (u32 i = 0; i < object_descriptor_capacity; i++)
                    layouts[i + pipeline_descriptor +
                            (pipeline_descriptor + object_descriptor_capacity) * f] = descriptor_set_layouts[0];
        if (n_pool_sizes[1])
            for (u32 f = 0; f < max_frames_in_flight; f++)
        ...
        *out_descriptor_sets = t_malloc<VkDescriptorSet>(pool_info.maxSets);
        ...
        VK_CRASH_CHECK(vkAllocateDescriptorSets(handle, &set_alloc_info, *out_descriptor_sets),
            ...
            for (const auto &descriptor: descriptors) {
                ...
                if (layout_binding.descriptorType != VK_DESCRIPTOR_TYPE_MAX_ENUM) {
                    ...
                    INTERNAL_ASSERT(false,
                                    "Different descriptor for the same binding in different shaders"); //TODO throw exception instead
                } else {
        ...
        for (auto &set: sets)
            set.erase(std::remove_if(set.begin(), set.end(),
                                     [](const VkDescriptorSetLayoutBinding &b) {
                                         return b.descriptorType == VK_DESCRIPTOR_TYPE_MAX_ENUM;
                                     }), set.end());
        ...
        for (auto &set: sets) {
            if (set.empty()) (*out_descriptor_set_layouts)[*out_n_descriptor_set_layouts] = VK_NULL_HANDLE;
                ...
                VK_CRASH_CHECK(vkCreateDescriptorSetLayout(handle, &layout_info, nullptr,
                                                           &(*out_descriptor_set_layouts)[*out_n_descriptor_set_layouts]),
        ...
        if (total_pipeline_sets) {
            ...
            if (sizes_count) {
                ...
                *out_object_descriptors = t_malloc<VkDescriptorType>(*out_n_object_descriptors);
                for (u32 i = 0; i < *out_n_object_descriptors; i++)
                    (*out_object_descriptors)[i] = sets[0][i].descriptorType;
            }
    ...
    graphics_pipeline::graphics_pipeline(device &owner, const std::vector<const shader *> &shaders,
        ...
        init_descriptors(owner.handle, shaders, n_descriptor_pool_sizes, descriptor_pool_sizes,
                         &n_descriptor_set_layouts, &descriptor_set_layouts, &frame_descriptors[0].descriptor_pool,
                         &descriptor_sets,
        ...
        */
        ...
        std::free(
                const_cast<VkVertexInputBindingDescription *>(vertex_input_info.pVertexBindingDescriptions)); //a bit messy, but shouldn't cause issues
        std::free(
                const_cast<VkVertexInputAttributeDescription *>(vertex_input_info.pVertexAttributeDescriptions)); //a bit messy, but shouldn't cause issues
    }
        ...
        if (n_descriptor_pool_sizes[0]) {
            if (push_data_size) {
                for (const auto &obj: objects) {
                    ...
                    vkCmdBindDescriptorSets(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, 1,
                                            &descriptor_sets[obj.properties().descriptor_set_idx +
                                                             frame_descriptor_count * current_frame],
    ...
    inline std::vector<VkWriteDescriptorSet> graphics_pipeline::generate_descriptor_write(u8 current_frame) {
        ...
        for (std::size_t i = 0; i < cached_object_bindings.size(); i++) {
            ...
            descriptor_write.dstSet = descriptor_sets[pipeline_descriptor + set +
                                                      frame_descriptor_count * current_frame]; //TODO chanage
        ...
        if (frame_descriptors[current_frame].dirty) {
            ...
            while (not_sorted) {
                ...
                while (j - i < 2) {
                    if (j < objects.size()) {
                        bool equal_set = true;
                        for (u32 a = i * n_object_bindings, b = j * n_object_bindings;
                             b < (j + 1) * n_object_bindings; a++, b++) {
            ...
            for (i = 0; i < duplicate_ranges.size(); i += 2) {
                cached_object_bindings.erase(
                        cached_object_bindings.begin() + (duplicate_ranges[i] - total_erased) * n_object_bindings,
            ...
            if (frame_descriptors[current_frame].object_set_cap < cached_object_bindings.size() / n_object_bindings) {
                ...
                } while (frame_descriptors[current_frame].object_set_cap <
                         cached_object_bindings.size() / n_object_bindings);
    ...
    buffer_binding_args
    graphics_pipeline::get_binding(const uniform &resource) const { return owner->get_memory().binding_args(resource); }
    ...
    buffer_binding_args graphics_pipeline::get_binding(
            const dynamic_storage_array &resource) const { return owner->get_memory().binding_args(resource); }
    ...
    buffer_binding_args graphics_pipeline::get_binding(
            const dynamic_storage_vector &resource) const { return owner->get_memory().binding_args(resource); }
