...
namespace ENGINE_NAMESPACE {
    inline void calc_queue_indices(VkPhysicalDevice &physical_handle, VkSurfaceKHR &surface,
                                   u32 *out_graphics_idx, u32 *out_present_idx, u32 *out_compute_idx,
        ...
        for (u32 i = 0; i < queue_family_count; i++) {
            if (queue_families[i].queueCount) {
                ...
                LOG_INTERNAL_INFO(
                        "[RENDERER] Queue family " << i << " properties: " << queue_families[i].queueCount << ' '
        ...
        LOGF_INTERNAL_INFO(
                "[RENDERER] Selected queue indices: graphics = {0} | present = {1} | compute = {2} | transfer = {3}",
    ...
    inline void create_logical_device(VkPhysicalDevice &physical_handle, VkDevice &handle,
        ...
        VK_CRASH_CHECK(vkCreateDevice(physical_handle, &createInfo, nullptr, &handle),
                       "Failed to create logical device handle");
    ...
    inline void create_command_buffers(VkDevice &handle, u32 command_parallelism,
                                       u32 graphics_idx, VkCommandPool **graphics_command_pools,
        ...
        *graphics_command_pools = t_malloc<VkCommandPool>(command_parallelism);
        ...
        for (u32 i = 0; i < command_parallelism; i++) VK_CRASH_CHECK(
                vkCreateCommandPool(handle, &poolInfo, nullptr, &(*graphics_command_pools)[i]),
                "Failed to create graphics command pool");

        const u32 buffers_per_frame = command_parallelism + 1;
        *graphics_command_buffers = t_malloc<VkCommandBuffer>(
                static_cast<std::size_t>(max_frames_in_flight) * buffers_per_frame);
        for (u32 f = 0; f < max_frames_in_flight; f++) {
            ...
            VK_CRASH_CHECK(vkAllocateCommandBuffers(handle, &mainAllocInfo,
                                                    &(*graphics_command_buffers)[buffers_per_frame * f]),
            ...
            for (u32 i = 0; i < command_parallelism; i++) {
                ...
                VK_CRASH_CHECK(vkAllocateCommandBuffers(handle, &subAllocInfo,
                                                        &(*graphics_command_buffers)[buffers_per_frame * f + i + 1]),
                               "Failed to allocate secondary graphics command buffer");
            }
        }
    }

    device::device(VkPhysicalDevice &&physical, VkSurfaceKHR &surface) : surface(&surface),
                                                                         physical_handle(std::move(physical)),
                                                                         command_parallelism(1), pipeline_stacks(
                    1) //TODO: + immediate_thread_num()
    {
        ...
        calc_queue_indices(physical_handle, surface, &graphics_idx, &present_idx, &compute_idx, &transfer_idx,
                           invalid_queue_idx);
        ...
        create_command_buffers(handle, command_parallelism, graphics_idx, &graphics_command_pools,
    ...
    bool device::draw() {
        ...
        for (auto &pipeline: graphics_pipelines)
            pipeline.update_descriptor_sets(previous_frame, current_frame,
                                            next_frame); //TODO consider parallelizing this
        ...
        u32 imageIndex;
        VkResult result = vkAcquireNextImageKHR(handle, main_swapchain.vk_handle(), UINT64_MAX,
        ...
        VK_CRASH_CHECK(vkBeginCommandBuffer(primary_buffer, &beginInfo),
                       "Failed to begin primary graphics command buffer");
        ...
        vkCmdBeginRenderPass(primary_buffer, &renderPassInfo,
        ...
        vkCmdExecuteCommands(primary_buffer, command_parallelism,
                             &graphics_command_buffers[(command_parallelism + 1) * current_frame + 1]);
        ...
        VK_CRASH_CHECK(vkQueueSubmit(graphics_queue, 1, &submitInfo, frame_fences[current_frame]),
        ...
        memory.sync(handle,
                    current_frame); //wait for the "next" frame's data transfers to finish, so nothing can be overwritten by the client
        memory.map_ranges(handle, current_frame);
        return true;
    }
