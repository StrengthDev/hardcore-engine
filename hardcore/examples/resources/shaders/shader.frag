...
{
    vec2 res = coords;
    uint iterations = buffers.frag_params.iterations;
...
{
    switch (buffers.frag_params.func_idx)
    {
        case 0:
        return func0(coords);
        case 1:
...
void main() {
    ...
    vec3 colors = hsv2rgb(vec3(ang, 1.0, fract(log2(rad) + buffers.frag_params.frame)));
