...
namespace ENGINE_NAMESPACE {
    ...
    void staging_pool::free(VkDevice device, device_heap_manager &heap_manager) {
        if (m_memory != VK_NULL_HANDLE) {
            ...
            heap_manager.free(<#initializer#>, <#initializer#>, device);
        }
    }
    ...
    void upload_pool::create_batch(u8 pool_type, std::size_t pool_idx, VkBuffer buffer) {
        ...
        m_pending_copies.insert({{pool_type, pool_idx},
                                 {buffer,    std::vector<VkBufferCopy>()}});
    }

    void
    upload_pool::submit(u8 pool_type, std::size_t pool_idx, const void *data, VkDeviceSize size, VkDeviceSize offset) {
    ...
    void texture_upload_pool::record_and_clear_transfer(VkCommandBuffer &buffer, u32 queue_idx) {
        if (m_pending_copies_transfer.empty())
            return;
        ...
        for (const transfer_data &data: m_pending_copies_transfer) {
            if (data.conversion == TEXEL_ONLY_CONVERSION) {
                barriers_0.push_back(layout_transition(data.src.image,
                                                       data.src_layout_0, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                                       data.aspect, queue_idx));
                barriers_1.push_back(layout_transition(data.src.image,
                                                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, data.src_layout_1,
                                                       data.aspect, queue_idx));
            }

            barriers_0.push_back(layout_transition(data.dst_image,
                                                   data.dst_layout_0, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, data.aspect,
                                                   queue_idx));
            barriers_1.push_back(layout_transition(data.dst_image,
                                                   VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, data.dst_layout_1, data.aspect,
                                                   queue_idx));

        }

        vkCmdPipelineBarrier(buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr,
                             0, nullptr, barriers_0.size(), barriers_0.data());

        for (const transfer_data &data: m_pending_copies_transfer) {
            switch (data.conversion) {
                case BUFFER_TO_IMAGE:
                    vkCmdCopyBufferToImage(buffer, data.src.buffer, data.dst_image,
        ...
        vkCmdPipelineBarrier(buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, nullptr,
                             0, nullptr, barriers_1.size(), barriers_1.data());
    ...
    void texture_upload_pool::record_and_clear_graphics(VkCommandBuffer &buffer, u32 queue_idx) {
        if (m_pending_copies_graphics.empty())
            return;
